	...
	class Tribe__Settings_Helper {
		...
		public function add_fields( $fields, $setting_tab, $neighboring_field = null, $above = false ) {
			if ( ! is_string( $neighboring_field ) ) {
				...
				$this->insert_fields_misc = array_replace_recursive( $this->insert_fields_misc, array( $setting_tab => $fields ) );
			} elseif ( true === $above ) {
				// Add to above fields list with neighbor specified.
				$this->insert_fields_above = array_replace_recursive( $this->insert_fields_above, array( $setting_tab => array( $neighboring_field => $fields ) ) );
			} else {
				// Add to below fields list with neighbor specified.
				$this->insert_fields_below = array_replace_recursive( $this->insert_fields_below, array( $setting_tab => array( $neighboring_field => $fields ) ) );
			}
